...
struct Buffer {
    ...
    template <typename T> T& Read(const uint64_t offset = 0) { return *(T*)((Byte*)Data + offset); }
...
struct SafeBuffer final : public Buffer {
    ~SafeBuffer() override { Buffer::Release(); }
...
struct SharedBuffer final : public Buffer {
    ...
    SharedBuffer(const SharedBuffer& other) : Buffer(other) {
