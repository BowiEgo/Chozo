# MeshComponent - Typed Mesh Component Documentation

## Overview

MeshComponent is a template-based mesh component that provides type-safe storage and management of mesh parameters. It supports compile-time type checking, automatic mesh data generation, caching, and integration with the plugin system.
...
MeshComponent<CubeParams> cube;

// Constructor with parameters
MeshComponent<CubeParams> cube(2.0f, 3.0f, 1.0f, 4, 4, 4);

// Constructor with params struct
CubeParams params;
params.Width = 2.0f;
params.Height = 3.0f;
MeshComponent<CubeParams> cube(params);
```
...
constexpr MeshType type = MeshComponent<CubeParams>::GetStaticType();

// Get mesh type at runtime
MeshType runtimeType = cube.GetType();
...
auto cube1 = MeshComponent<CubeParams>(2.0f, 2.0f, 2.0f);
auto cube2 = MeshComponent<CubeParams>(2.0f, 2.0f, 2.0f);
if (cube1 == cube2) {
// Same parameters
}
...
void ExampleUsage() {
// Create components
auto cube = MeshComponent<CubeParams>(2.0f, 2.0f, 2.0f);
auto sphere = MeshComponent<SphereParams>(1.0f, 32, 16);
...
| :------- | :--------------- | :---------------------------------------------------------------------------------- |
| Cube     | CubeParams       | Width, Height, Depth, WidthSegments, HeightSegments, DepthSegments                  |
| Sphere   | SphereParams     | Radius, WidthSegments, HeightSegments, PhiStart, PhiLength, ThetaStart, ThetaLength |
...
- MeshComponent is not thread-safe by default
...
```cpp
// In editor UI
void DrawMeshComponentUI(MeshComponent<SphereParams>& sphere) {
...
```cpp
using FCubeComponent = MeshComponent<CubeParams>;
using FSphereComponent = MeshComponent<SphereParams>;
```
