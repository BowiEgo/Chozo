...
DEFINE_LOG_CATEGORY_STATIC(LogConsolePanel, Info);
...
ConsolePanel::ConsolePanel() {
    m_LogSink = CZ_CREATE_SCOPE(
        MEMORY_USAGE_UI, LogScopedSink,
        [this](const std::string& message, LogVerbosity verbosity) {
            this->AddLog(message.c_str(), verbosity);
        },
        "[%T] [%n] [%l]: %v");
...
void ConsolePanel::Draw(const char* title) {
    ...
    if (ImGui::SmallButton("Add Debug Text")) {
        CZ_LOG(LogConsolePanel, Trace, "some more text");
    }
    ImGui::SameLine();
    if (ImGui::SmallButton("Add Debug Error")) {
        CZ_LOG(LogConsolePanel, Error, "some error text");
    }
    ImGui::SameLine();
    ...
    if (ImGui::BeginChild("ScrollingRegion", ImVec2(0, -footer_height_to_reserve),
        ...
        for (int i = 0; i < m_Items.Size; i++) {
            const LogLine& item = m_Items[i];

            if (!m_Filter.PassFilter(item.Text)) continue;
            ImVec4 color = GetColorForLevel(item.Level);
...
void ConsolePanel::ClearLog() {
    for (int i = 0; i < m_Items.Size; i++) {
        if (m_Items[i].Text) {
            ImGui::MemFree((void*)m_Items[i].Text);
        }
    }
    m_Items.clear();
}

void ConsolePanel::AddLog(const char* message, LogVerbosity level) {
    std::scoped_lock<std::mutex> lock(m_LogMutex);
    m_Items.push_back({ Strdup(message), level });
...
void ConsolePanel::ExecCommand(const char* command_line) {
    ...
    if (Stricmp(command_line, "CLEAR") == 0) {
        ...
        CZ_LOG(LogConsolePanel, Trace, "m_Commands:");

        for (int i = 0; i < m_Commands.Size; i++)
            CZ_LOG(LogConsolePanel, Trace, "- {}", m_Commands[i]);
    } else if (Stricmp(command_line, "HISTORY") == 0) {
        int first = m_History.Size - 10;
        for (int i = first > 0 ? first : 0; i < m_History.Size; i++)
            CZ_LOG(LogConsolePanel, Trace, "{}: {}\n", i, m_History[i]);
    } else {
        CZ_LOG(LogConsolePanel, Warning, "Unknown command: {}\n", command_line);
    }
...
ImVec4 ConsolePanel::GetColorForLevel(LogVerbosity level) {
    switch (level) {
        case LogVerbosity::Fatal: return ImVec4(1.0f, 0.2f, 0.2f, 1.0f);   // Bright Red
        case LogVerbosity::Error: return ImVec4(1.0f, 0.4f, 0.4f, 1.0f);   // Soft Red
        case LogVerbosity::Warning: return ImVec4(1.0f, 0.8f, 0.0f, 1.0f); // Gold/Yellow
        case LogVerbosity::Info: return ImVec4(0.2f, 1.0f, 0.4f, 1.0f);    // Green
        case LogVerbosity::Debug: return ImVec4(0.3f, 0.7f, 1.0f, 1.0f);   // Sky Blue
        case LogVerbosity::Trace:
        default: return ImVec4(1.0f, 1.0f, 1.0f, 1.0f); // Grey
    }
}