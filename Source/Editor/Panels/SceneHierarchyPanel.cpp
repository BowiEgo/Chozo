...
void SceneHierarchyPanel::FlattenTree(EditorNode* node, int depth) {
    if (!node) return;
    ...
    if (m_Filter.IsActive()) {
        ...
        for (EditorNode* child : node->GetChildren()) {
...
void SceneHierarchyPanel::FlattenTreeFiltered(EditorNode* node, int depth) {
    if (!node) return;
...
void SceneHierarchyPanel::DrawFlattenedNode(EditorNode* node, int depth) {
    ImGui::TableNextRow();
    ...
    if (ImGui::IsItemHovered() && ImGui::IsMouseClicked(1)) {
        CZ_LOG(LogSceneHierarchyPanel, Trace, "Right-clicked Node: {} (ID: {}, Type: {})",
               node->GetName().c_str(), node->GetID(),
               TypeRegister::Get().TypeMaskToString(node->GetTypeMask()));
    }
...
void SceneHierarchyPanel::CreateNode(const std::string name, const TypeMask typeMask,
                                     EditorNode* parent) {
    auto newNode = m_NodeTree->CreateNode(name, typeMask, parent);
