...
TEST_SUITE("HeapMalloc / HeapFree") {
    ...
    TEST_CASE("Basic allocation and free") {
        ...
        void* ptr             = HeapMalloc(size, MEMORY_USAGE_MISC);
        REQUIRE(ptr != nullptr);
    ...
    TEST_CASE("Multiple allocations across usages") {
        void* a = HeapMalloc(32, MEMORY_USAGE_RENDER);
        void* b = HeapMalloc(48, MEMORY_USAGE_ASSET);
        REQUIRE(a);
        REQUIRE(b);
...
TEST_SUITE("MemoryProfile and leak detection") {
    ...
    TEST_CASE("Peak tracking") {
        ...
        void* a = HeapMalloc(64, MEMORY_USAGE_UI);
        void* b = HeapMalloc(256, MEMORY_USAGE_UI);
        REQUIRE(a);
        REQUIRE(b);
    ...
    TEST_CASE("Leak detection") {
        ...
        void* leak = HeapMalloc(100, MEMORY_USAGE_SCENE);
        REQUIRE(leak);
...
TEST_SUITE("New / Delete template functions") {
    ...
    TEST_CASE("New and Delete with usage") {
        ...
        auto* obj = New<TestObj>(MEMORY_USAGE_JOB_SYSTEM, 77);
        REQUIRE(obj != nullptr);
...
TEST_SUITE("StlAllocator") {
    ...
    TEST_CASE("Vector with StlAllocator") {
        REQUIRE_ZERO(MEMORY_USAGE_PHYSICS);
        {
            ...
            CHECK_GE(GetMemoryProfile(MEMORY_USAGE_PHYSICS).Current, vec.capacity() * sizeof(int));
        }
...
TEST_SUITE("TypeMemoryTracker") {
    ...
    TEST_CASE("Leak detection per type") {
        ...
        bool found = false;
        for (int i = 0; i < count; ++i) {
            if (leaks[i].Current == 200) { // simple check; could extend Usage field (not used here)
                found = true;
                break;
            }
        }
