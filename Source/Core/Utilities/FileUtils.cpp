...
std::filesystem::path GetProjectRoot() {
    ...
    try {
        ...
        CZ_CORE_LOG(Error, "Error while determining project root: {}", e.what());
    }
    ...
    if (in) {
        ...
        if (size > 0) {
            ...
            if (!in.read(result.data(), size)) {
                CZ_CORE_LOG(Error, "Failed to read content from file: {0}", filepath.string());
                result.clear();
            }
        } else {
            CZ_CORE_LOG(Warning, "File is empty: {0}", filepath.string());
        }
        in.close();
    } else {
        CZ_CORE_LOG(Error, "Could not open file '{0}'", filepath.string());
    }
...
bool ReadBinaryFile(const std::string& filepath, std::vector<uint32_t>& target) {
    ...
    if (in) {
        ...
        CZ_CORE_LOG(Error, "Could not open file '{0}'", filepath);
        return false;
    }
}
...
void DeleteFile(const std::string& filepath) {
    try {
        if (std::filesystem::exists(filepath)) {
            ...
            CZ_CORE_LOG(Trace, "File at {} deleted successfully", filepath);
        } else {
            CZ_CORE_LOG(Warning, "File at {} not found", filepath);
        }
    } catch (const std::filesystem::filesystem_error& err) {
        CZ_CORE_LOG(Error, "Error: {}  {}", err.what(), filepath);
    }
}
