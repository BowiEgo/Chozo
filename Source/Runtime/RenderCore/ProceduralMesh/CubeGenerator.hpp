...
public:
    MeshBuffer* GenerateBuffer(MeshParams params, MeshBuffer* buffer) override {
        ...
        auto cubeParams = params.As<CubeParamsObj>();

        // build each side of the box geometry
        BuildPlane(buffer, "z", "y", "x", -1, -1, cubeParams->Depth, cubeParams->Height,
                   cubeParams->Width, cubeParams->DepthSegments, cubeParams->HeightSegments,
                   totalVertexCounter); // px
        BuildPlane(buffer, "z", "y", "x", 1, -1, cubeParams->Depth, cubeParams->Height,
                   -cubeParams->Width, cubeParams->DepthSegments, cubeParams->HeightSegments,
                   totalVertexCounter); // nx
        BuildPlane(buffer, "x", "z", "y", 1, 1, cubeParams->Width, cubeParams->Depth,
                   cubeParams->Height, cubeParams->WidthSegments, cubeParams->DepthSegments,
                   totalVertexCounter); // py
        BuildPlane(buffer, "x", "z", "y", 1, -1, cubeParams->Width, cubeParams->Depth,
                   -cubeParams->Height, cubeParams->WidthSegments, cubeParams->DepthSegments,
                   totalVertexCounter); // ny
        BuildPlane(buffer, "x", "y", "z", 1, -1, cubeParams->Width, cubeParams->Height,
                   cubeParams->Depth, cubeParams->WidthSegments, cubeParams->HeightSegments,
                   totalVertexCounter); // pz
        BuildPlane(buffer, "x", "y", "z", -1, -1, cubeParams->Width, cubeParams->Height,
                   -cubeParams->Depth, cubeParams->WidthSegments, cubeParams->HeightSegments,
                   totalVertexCounter); // nz

        return buffer;
    }

private:
    void BuildPlane(MeshBuffer* buffer, std::string u, std::string v, std::string w, int uDir,
                    int vDir, float width, float height, float depth, uint32 gridX, uint32 gridY,
        ...
        for (uint32 iy = 0; iy < gridY1; iy++) {
            ...
            for (uint32 ix = 0; ix < gridX1; ix++) {
                ...
                buffer->Vertices.push_back(vertice);
                vertexCounter += 1;
            }
        }
