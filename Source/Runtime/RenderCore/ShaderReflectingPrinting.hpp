...
#define SCOPED_OBJECT() ScopedObject scopedObject##__COUNTER__(this)
    ...
    CumulativeOffset calculateCumulativeOffset(slang::ParameterCategory layoutUnit,
        ...
        switch (layoutUnit) {
            ...
            case slang::ParameterCategory::DescriptorTableSlot:
                ...
                for (auto node = accessPath.deepestParameterBlock; node != nullptr;
                     node      = node->outer) {
    ...
    SlangResult collectEntryPointMetadata(slang::IComponentType* program, int targetIndex,
                                          int entryPointCount) {
    ...
    std::vector<ComPtr<slang::IMetadata>> _metadataForEntryPoints;
    ...
    StageMask calculateParameterStageMask(slang::ParameterCategory layoutUnit,
        ...
        for (int i = 0; i < entryPointCount; ++i) {
            ...
            _metadataForEntryPoints[i]->isParameterLocationUsed(SlangParameterCategory(layoutUnit),
                                                                offset.space, offset.value, isUsed);
            if (isUsed) {
    ...
    StageMask calculateStageMask(slang::VariableLayoutReflection* variableLayout,
                                 AccessPath accessPath) {
        StageMask mask = 0;
