...
vec3 SpecularDFG(BRDFContext BRDFCtx)
{
//    return BRDFCtx.f0 * BRDFCtx.dfg.x + BRDFCtx.dfg.y;
    return mix(BRDFCtx.dfg.xxx, BRDFCtx.dfg.yyy, vec3(0.1));
}

float PerceptualRoughnessToLod(BRDFContext BRDFCtx, float perceptualRoughness)
{
...
vec3 DiffuseIrradiance(const vec3 n, BRDFContext BRDFCtx)
{
...
//    BRDFCtx.f0 = mix(vec3(GBuffer.Reflectance), GBuffer.BaseColor, GBuffer.Metallic);
    BRDFCtx.dfg = textureLod(u_BRDFLutTex, vec2(BRDFCtx.NoV, GBuffer.PerceptualRoughness), 0.0).rgb;

//    BRDFCtx.EnergyCompensation = 1.0 + BRDFCtx.f0 * (1.0 / BRDFCtx.dfg.y - 1.0);
