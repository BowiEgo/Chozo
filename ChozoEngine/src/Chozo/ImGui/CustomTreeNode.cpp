...
namespace ImGui {
    ...
    const ImU32 arrow_col = selected ? IM_COL32(26, 26, 26, 255) : IM_COL32(77, 77, 77, 255);

		ImGuiNavHighlightFlags nav_highlight_flags = ImGuiNavHighlightFlags_None;
		if (display_frame)
		{
			// Framed type
			...
			if (icon)
			{
                ...
                ImGui::SetCursorPosY(ImGui::GetCursorPosY() - frame_height + pad);
                ImGui::SetCursorPosX(ImGui::GetCursorPosX() + arrowWidth);
				ImGui::Image((ImTextureID)(uintptr_t)icon->GetRendererID(), { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 0), ImVec2(1, 1), iconTint /*selected ? colourDark : tintFloat*/);
		...
		else
		{
			...
			if (icon)
			{
				...
				const float arrowWidth = 20.0f + 1.0f;
				auto cursorPos = ImGui::GetCursorPos();
                ImGui::SetCursorPosY(ImGui::GetCursorPosY() - frame_height);
                ImGui::SetCursorPosX(ImGui::GetCursorPosX() + arrowWidth);
				ImGui::Image((ImTextureID)(uintptr_t)icon->GetRendererID(), { frame_height - pad * 2.0f, frame_height - pad * 2.0f }, ImVec2(0, 1), ImVec2(1, 0), iconTint /*selected ? colourDark : tintFloat*/);
