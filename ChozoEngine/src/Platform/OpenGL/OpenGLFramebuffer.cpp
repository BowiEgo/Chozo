...
namespace Chozo {
    ...
    // TODO: fix issue
    void OpenGLFramebuffer::CreateColorAttachmentImages(int samples, std::vector<FramebufferTextureSpecification> attachmentSpecs, uint32_t width, uint32_t height)
    {
    ...
    void OpenGLFramebuffer::CreateDepthAttachmentImage(int samples, FramebufferTextureSpecification attachmentSpec, uint32_t width, uint32_t height)
    {
        bool multisampled = samples > 1;
        GLenum target = multisampled ? GL_TEXTURE_2D_MULTISAMPLE : GL_TEXTURE_2D;
